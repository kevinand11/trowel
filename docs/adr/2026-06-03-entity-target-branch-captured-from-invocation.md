...
`trowel start` and `trowel fix` now store a **Target branch** on the entity: the branch the user was on for the invocation that materialised the PRD or Fix. Trowel creates the entity's working branch from that target and Close-out ships the entity back into that same target, either by opening a PR against it (`usePrs: true`) or by host-merging into it (`usePrs: false`).
