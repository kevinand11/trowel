...
- **No mutation lock; rely on user discipline.** Rejected: trowel is single-user, but it's not single-terminal. A backgrounded `trowel change work` plus an interactive `trowel close prd 3` can absolutely race; the compute-on-demand id allocation would silently double-issue. The lock is the cost of choosing compute-on-demand over a persisted counter.
...
`trowel change work` and `trowel change start` are unchanged (they already carried both flags).
